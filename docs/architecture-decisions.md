...
| DQ-004 | Low-confidence edge threshold. | Design lean was a 5.0 default, configurable. **Never implemented** (recorded 2026-09-11): no numeric threshold exists in code. Low-confidence edges are reviewed through the semantic-edge candidate queue, and every candidate requires explicit human acceptance. Still open. |
| DQ-005 | Contradictions as first-class edges. | `contradicts` is a first-class reviewed relation. Candidates are queued with dates/provenance only when there is disagreement evidence: a negation/refutation term on one side only, over shared claim wording (overlap ≥ 0.4), with embedding similarity in [0.6, 0.97). Similarity only bounds the scan (denial lowers similarity, so a high floor is anti-correlated with contradiction); shared labels and origin-date delta only break ties. The lexical rule is only a pre-filter: before anything is queued, a local model must confirm each pair (`runtime.contradiction_confirmation_*`, on by default, fails closed when the model is unavailable). Measured 2026-09-11 on `mnemosyne_dev`, the rule alone was about 0.25% precise. Conflict resolution workflows remain later. |
