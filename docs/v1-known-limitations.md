...
- Contradiction candidates pass a lexical pre-filter (one-sided negation over shared claim wording), then need local-model confirmation before anything is queued. Measured 2026-09-11 on `mnemosyne_dev` (bge-small embeddings), the similarity floor is effectively inert because neighbour similarity is compressed high, and the lexical rule alone was about 0.25% precise. The confirmation model (45/48 on labelled pairs) is what keeps the review queue usable. The rule misses contradictions that use no negation words. Re-measure after changing the embedding or confirmation model.
