...
- Conservative `contradicts` candidates (`candidate_source: contradiction_signals`) reuse that review queue. Generation requires a high embedding-similarity band plus at least two of shared semantic labels, origin-date delta, and conflict-lexicon hits; candidate rows stamp both nodes' dates and provenance.
